...
BOOL dbaccess::sqlconn(UCHAR * tbname,UCHAR * login,UCHAR * pass)
{
   ...
   rc = SQLConnect(hdbc,(SQLCHAR*) "buf",SQL_NTS,(SQLCHAR*) login,SQL_NTS,(SQLCHAR*) pass,SQL_NTS);
   ...
   if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) { SQLDisconnect(hdbc); SQLFreeHandle (SQL_HANDLE_ENV, henv); SQLFreeHandle(SQL_HANDLE_DBC, hdbc); return FALSE; }
   //UL_INFO((LOGID1, "sqlconn :: SQLConnect(hdbc,%s,SQL_NTS,(SQLCHAR*) %s,SQL_NTS,(SQLCHAR*) %s,SQL_NTS) = rc",chr_ds_name,login,pass,rc));
   if (!MYSQLSUCCESS(rc))
   {
	  UCHAR State[10];
		 ...
		 while (TRUE)
			{
			 ...
			 if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
				...
				{
				 //UL_INFO((LOGID1, "total %d rows",pos));
				 return pos;
				}
			}	 
		}
...
//   UL_INFO((LOGID1, "sqldisconn :: SQLFreeConnect %d",retcode));   
//   retcode=SQLFreeEnv(henv);
//   UL_INFO((LOGID1, "sqldisconn :: SQLFreeEnv %d",retcode));
